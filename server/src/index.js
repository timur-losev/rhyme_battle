...
app.use(cors({
  origin: '*', // Разрешаем все origins для отладки
  methods: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS'],
  credentials: false // Отключаем credentials для упрощения
}));
...
io.on('connection', (socket) => {
  ...
  socket.on('getRoomState', (data) => {
    ...
    try {
      ...
      const state = gameService.getRoomState(roomId);
      ...
      // Добавляем дополнительную информацию о состоянии
